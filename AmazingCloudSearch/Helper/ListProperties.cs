...
namespace AmazingCloudSearch.Helper
{
    class ListProperties<T>
    {
        ...
        public List<PropertyInfo> List()
        {
            ...
            {
                object[] customA = p.GetCustomAttributes(typeof(JsonPropertyAttribute), true);
                if (customA.Length == 0)
                    continue;
                //Newtonsoft.Json.JsonPropertyAttribute jsonAttribute = (Newtonsoft.Json.JsonPropertyAttribute)customA[0];
