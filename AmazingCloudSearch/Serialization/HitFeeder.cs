    ...
    class HitFeeder<T> where T : SearchDocument, new()
    {
        ...
        public void Feed(SearchResult<T> searchResult, dynamic dyHit)
        {
            ...
            foreach(var hitDocument in dyHit)
            {
                ...
                string hitFieldsStr = hitDocument.fields.ToString();

                T jsonDynamic = JsonConvert.DeserializeObject<T>(hitFieldsStr);

                
                //Dictionary<string, List<string>> jsonHitField = _serializer.Deserialize<Dictionary<string, List<string>>>(hitFieldsStr);

                //T hit = Map(jsonHitField);

                searchResult.hits.hit.Add(new SearchResult<T>.Hit<T> { id = hitDocument.id, data = jsonDynamic });
            }
        }
